const daneJsonUjemne = {
  ...
  p18: {
    pozycja:
      "Za każdą opuszczoną godzinę bez usprawiedliwienia w dniu odrabiania zajęć szkolnych",
    punkty: -5,
  },
...
const daneJsonDodatnie = {
  ...
  p10: {
    pozycja:
      "Do dyspozycji wychowawców klas programowo najwyższych po konsultacji z zespołem nauczycieli",
    punkty: 25,
  },
  p11: {
    pozycja:
      "Stosunek do obowiązków szkolnych po konsultacji z zespołem nauczycieli",
    punkty: 15,
  },
  p12: {
...
function Popieranie_danych(d, formId) {
  ...
  Object.keys(d).forEach((key) => {
    ...
    if (typeof value.punkty === "object") {
      ...
      const srednia = (
        punktyArray.reduce((acc, curr) => acc + curr, 0) / punktyArray.length
      ).toFixed(2);
      z = srednia;
    } else {
      z = value.punkty;
...
Popieranie_danych(daneJsonDodatnie, "a0");
